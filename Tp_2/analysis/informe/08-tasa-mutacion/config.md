# Configuración — Carga de mutación

Barrido `tasa_mutacion` · batch `20260906T233756Z` · commit `5b59273` (árbol sucio)

- **Semillas**: [1, 2, 3, 4, 5]
- **Targets**: `plana` = `images/japan.png`, `detallada` = `images/pika.png`, `compleja` = `images/noche_estrellada.jpg`
- **Varía**: `mutation`, `mutation_rate`
...
| Variante | Qué cambia |
|---|---|
| **carga 0.5** | `mutation` = `uniform` · `mutation_rate` = `0.001` |
| **carga 2** | `mutation` = `uniform` · `mutation_rate` = `0.004` |
| **carga 4** | `mutation` = `uniform` · `mutation_rate` = `0.008` |
| **carga 10** | `mutation` = `uniform` · `mutation_rate` = `0.02` |
| **carga 25** | `mutation` = `uniform` · `mutation_rate` = `0.05` |
| **carga 50** | `mutation` = `uniform` · `mutation_rate` = `0.1` |

## Reproducir

```bash
python analysis/run_experiments.py tasa_mutacion --clean
