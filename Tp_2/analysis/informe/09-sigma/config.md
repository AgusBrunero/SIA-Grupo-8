# Configuración — Magnitud de la mutación (σ)

Barrido `sigma` · batch `20260906T233756Z` · commit `5b59273` (árbol sucio)

- **Semillas**: [1, 2, 3, 4, 5]
- **Targets**: `plana` = `images/japan.png`, `detallada` = `images/pika.png`, `compleja` = `images/noche_estrellada.jpg`
- **Varía**: `mutation_sigma`
...
| Variante | Qué cambia |
|---|---|
| **sigma=0.02** | `mutation_sigma` = `0.02` |
| **sigma=0.05** | `mutation_sigma` = `0.05` |
| **sigma=0.10** | `mutation_sigma` = `0.1` |
| **sigma=0.20** | `mutation_sigma` = `0.2` |
| **sigma=0.40** | `mutation_sigma` = `0.4` |

## Reproducir

```bash
python analysis/run_experiments.py sigma --clean
