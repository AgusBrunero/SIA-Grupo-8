# Configuración — Presión de selección

Barrido `presion` · batch `20260906T233756Z` · commit `5b59273` (árbol sucio)

- **Semillas**: [1, 2, 3, 4, 5]
- **Targets**: `plana` = `images/japan.png`, `detallada` = `images/pika.png`, `compleja` = `images/noche_estrellada.jpg`
- **Varía**: `selection_parents`, `tournament`
...
| Variante | Qué cambia |
|---|---|
| **torneo M=2** | `selection_parents` = `tournament_det` · `tournament` = `{"m": 2, "threshold": 0.75}` |
| **torneo M=4** | `selection_parents` = `tournament_det` · `tournament` = `{"m": 4, "threshold": 0.75}` |
| **torneo M=10** | `selection_parents` = `tournament_det` · `tournament` = `{"m": 10, "threshold": 0.75}` |
| **torneo M=30** | `selection_parents` = `tournament_det` · `tournament` = `{"m": 30, "threshold": 0.75}` |
| **torneo prob Th=0.55** | `selection_parents` = `tournament_prob` · `tournament` = `{"m": 4, "threshold": 0.55}` |
| **torneo prob Th=0.75** | `selection_parents` = `tournament_prob` · `tournament` = `{"m": 4, "threshold": 0.75}` |
| **torneo prob Th=0.95** | `selection_parents` = `tournament_prob` · `tournament` = `{"m": 4, "threshold": 0.95}` |

## Reproducir

```bash
python analysis/run_experiments.py presion --clean
