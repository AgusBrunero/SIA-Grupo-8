# Configuración — Selección combinada A%/B%

Barrido `seleccion_combinada` · batch `20260906T233756Z` · commit `5b59273` (árbol sucio)
...
| Variante | Qué cambia |
|---|---|
| **torneo det (puro)** | `selection_parents` = `tournament_det` |
| **elite (puro)** | `selection_parents` = `elite` |
| **ruleta (pura)** | `selection_parents` = `roulette` |
| **25% elite + 75% torneo** | `selection_parents` = `{"method_a": "elite", "method_b": "tournament_det", "a_ratio": 0.25}` |
| **50% elite + 50% torneo** | `selection_parents` = `{"method_a": "elite", "method_b": "tournament_det", "a_ratio": 0.5}` |
| **50% elite + 50% ruleta** | `selection_parents` = `{"method_a": "elite", "method_b": "roulette", "a_ratio": 0.5}` |
| **50% torneo + 50% ruleta** | `selection_parents` = `{"method_a": "tournament_det", "method_b": "roulette", "a_ratio": 0.5}` |

## Reproducir

```bash
python analysis/run_experiments.py seleccion_combinada --clean
